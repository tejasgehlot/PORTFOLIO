    ...
    <div className="case-study-page" style={{ background: 'var(--bg)', color: 'var(--text)', minHeight: '100dvh' }}>
      ...
      <section style={{ padding: '8rem 3rem 5rem', borderBottom: '1px solid var(--border)' }}>
        <motion.div initial="hidden" animate="visible" variants={containerVariants} className="case-hero-grid" style={{ display: 'grid', gridTemplateColumns: '1.1fr 0.9fr', gap: '3rem', alignItems: 'center', maxWidth: '1240px', margin: '0 auto' }}>
          <div style={{ paddingTop: '2rem' }}>
            <motion.div variants={fadeIn} custom={0} style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '0.72rem', textTransform: 'uppercase', letterSpacing: '0.18em', color: 'var(--text3)', marginBottom: '0.8rem' }}>InvoiceFlow · Multi-Tenant Billing SaaS</motion.div>
            ...
            <motion.div variants={fadeIn} custom={4} style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', marginBottom: '2rem' }}>
              {['Java', 'Spring Boot', 'Spring Security', 'JWT', 'Hibernate', 'MySQL', 'React', 'TypeScript', 'Tailwind CSS', 'Axios', 'Recharts', 'iText PDF', 'Vite'].map((tech) => (
                <span key={tech} style={{ padding: '0.5rem 0.9rem', border: '1px solid var(--border)', borderRadius: '999px', fontFamily: "'JetBrains Mono', monospace", fontSize: '0.68rem', letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--text2)' }}>{tech}</span>
              ))}
            </motion.div>
            <motion.div variants={fadeIn} custom={5} style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
              <a href="https://github.com/tejasgehlot/InvoiceFlow.git" target="_blank" rel="noreferrer" style={{ padding: '0.8rem 1.4rem', background: 'var(--text)', color: 'var(--bg)', border: 'none', borderRadius: '6px', fontFamily: "'JetBrains Mono', monospace", fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.1em', display: 'inline-flex', alignItems: 'center', gap: '0.5rem', textDecoration: 'none' }}>
                <Github size={14} />
                GitHub
              </a>
              <button style={{ padding: '0.8rem 1.4rem', background: 'transparent', color: 'var(--text)', border: '1px solid var(--border)', borderRadius: '6px', fontFamily: "'JetBrains Mono', monospace", fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.1em', cursor: 'pointer' }}>Live Demo</button>
            </motion.div>
          </div>
      ...
      <section style={{ padding: '5rem 3rem', borderBottom: '1px solid var(--border)' }}>
        ...
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="case-two-col" style={{ maxWidth: '1120px', margin: '0 auto', display: 'grid', gridTemplateColumns: '1.1fr 0.9fr', gap: '2rem', alignItems: 'start' }}>
          ...
          <div style={{ border: '1px solid var(--border)', padding: '2rem', background: 'var(--card-bg)' }}>
            <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '0.72rem', textTransform: 'uppercase', letterSpacing: '0.14em', color: 'var(--text3)', marginBottom: '1rem' }}>Integrations</div>
      ...
      <section style={{ padding: '5rem 3rem', borderBottom: '1px solid var(--border)' }}>
        ...
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="case-two-col" style={{ maxWidth: '1120px', margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 0.8fr', gap: '2rem' }}>
          ...
          <div style={{ border: '1px solid var(--border)', padding: '2rem', background: 'var(--card-bg)' }}>
            <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '0.72rem', letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--text3)', marginBottom: '1rem' }}>Project scope</div>
            <div style={{ display: 'grid', gap: '0.7rem', color: 'var(--text2)' }}>
          ...
          <div style={{ display: 'grid', gap: '0.8rem' }}>
            {['Login Request', 'BCrypt Password Verification', 'JWT Generation', 'Token stored in Browser', 'Axios Interceptor', 'JwtFilter Validation', 'AdminId Extraction', 'Business Service', 'Database Query filtered by adminId', 'Response'].map((step, idx) => (
              <div key={step} className="case-flow-step" style={{ display: 'flex', alignItems: 'center', gap: '0.8rem' }}>
                <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '0.72rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--text3)', minWidth: '70px' }}>{idx + 1}</div>
        ...
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="case-two-col" style={{ maxWidth: '1120px', margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
          {['Admin A', 'Admin B'].map((admin) => (
            <div key={admin} style={{ border: '1px solid var(--border)', padding: '1.5rem', background: 'var(--card-bg)' }}>
              <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '0.72rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--text3)', marginBottom: '0.6rem' }}>{admin}</div>
      ...
      <section style={{ padding: '5rem 3rem', borderBottom: '1px solid var(--border)' }}>
        ...
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} style={{ maxWidth: '900px', margin: '0 auto', display: 'grid', gap: '1rem' }}>
          {timeline.map(([day, label], idx) => <div key={day} className="case-timeline-row" style={{ display: 'flex', alignItems: 'center', gap: '1rem', borderLeft: '2px solid var(--border)', paddingLeft: '1.5rem' }}><div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '0.72rem', letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--text3)', minWidth: '80px' }}>{day}</div><div style={{ color: 'var(--text2)' }}>{label}</div></div>)}
        </motion.div>
      </section>

      <section style={{ padding: '6rem 3rem', textAlign: 'center' }}>
        <motion.h2 initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} style={{ fontSize: '2.5rem', fontWeight: 700, marginBottom: '2rem', textTransform: 'uppercase', fontFamily: "'JetBrains Mono', monospace", letterSpacing: '0.1em' }}>Ready to Build?</motion.h2>
        <motion.div initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} style={{ display: 'flex', flexDirection: 'column', gap: '1rem', maxWidth: '400px', margin: '0 auto 3rem' }}>
          <a href="https://github.com/tejasgehlot/InvoiceFlow.git" target="_blank" rel="noreferrer" style={{ padding: '0.75rem 1.5rem', width: '100%', background: 'var(--text)', color: 'var(--bg)', border: 'none', borderRadius: '6px', fontFamily: "'JetBrains Mono', monospace", fontSize: '0.75rem', letterSpacing: '0.1em', textTransform: 'uppercase', cursor: 'pointer', transition: 'opacity 0.2s', textDecoration: 'none', textAlign: 'center' }}>View GitHub Repository</a>
          <button style={{ padding: '0.75rem 1.5rem', width: '100%', background: 'transparent', color: 'var(--text)', border: '1px solid var(--border)', borderRadius: '6px', fontFamily: "'JetBrains Mono', monospace", fontSize: '0.75rem', letterSpacing: '0.1em', textTransform: 'uppercase', cursor: 'pointer', transition: 'all 0.2s' }}>Live Demo</button>
          <button style={{ padding: '0.75rem 1.5rem', width: '100%', background: 'transparent', color: 'var(--text)', border: '1px solid var(--border)', borderRadius: '6px', fontFamily: "'JetBrains Mono', monospace", fontSize: '0.75rem', letterSpacing: '0.1em', textTransform: 'uppercase', cursor: 'pointer', transition: 'all 0.2s' }}>Architecture Diagram</button>
        </motion.div>
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} style={{ paddingTop: '2rem', borderTop: '1px solid var(--border)', color: 'var(--text3)', fontFamily: "'JetBrains Mono', monospace", fontSize: '0.75rem', letterSpacing: '0.1em', textTransform: 'uppercase' }}>Built with Java • Spring Boot • React • TypeScript • Tailwind CSS • MySQL • Axios • Recharts • iText PDF • Vite</motion.div>
      </section>
    </div>
  )
}
